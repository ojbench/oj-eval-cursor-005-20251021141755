...
bool QoiEncode(uint32_t width, uint32_t height, uint8_t channels, uint8_t colorspace) {
    ...
    for (int i = 0; i < px_num; ++i) {
        ...
        if (r == pre_r && g == pre_g && b == pre_b && a == pre_a) {
            ++run;
            // Flush run if reaching maximum encodable length or last pixel
            if (run == 62 || i == px_num - 1) {
                QoiWriteU8(static_cast<uint8_t>(QOI_OP_RUN_TAG | (run - 1)));
                run = 0;
            }
        } else {
            // If there is a pending run, flush it before encoding current pixel
            if (run > 0) {
                QoiWriteU8(static_cast<uint8_t>(QOI_OP_RUN_TAG | (run - 1)));
                run = 0;
            }

            // Compute index and check QOI_OP_INDEX
            int idx = QoiColorHash(r, g, b, a);
            if (history[idx][0] == r && history[idx][1] == g && history[idx][2] == b && history[idx][3] == a) {
                QoiWriteU8(static_cast<uint8_t>(QOI_OP_INDEX_TAG | (idx & 0x3f)));
            } else if (a == pre_a) {
                // Try QOI_OP_DIFF
                ...
                if (dr >= -2 && dr <= 1 && dg >= -2 && dg <= 1 && db >= -2 && db <= 1) {
                    uint8_t diff_byte = static_cast<uint8_t>(QOI_OP_DIFF_TAG |
                        ((dr + 2) << 4) |
                        ((dg + 2) << 2) |
                        ((db + 2) << 0));
                    ...
                    if (dg_l >= -32 && dg_l <= 31 && dr_dg >= -8 && dr_dg <= 7 && db_dg >= -8 && db_dg <= 7) {
                        ...
                        // Fallback to QOI_OP_RGB or QOI_OP_RGBA
                        if (a == pre_a) {
            ...
            // Update history table for current pixel
            history[idx][0] = r;
            history[idx][1] = g;
            history[idx][2] = b;
            history[idx][3] = a;
    ...
    // Flush any remaining run (safety, though loop handles end flush)
    if (run > 0) {
        QoiWriteU8(static_cast<uint8_t>(QOI_OP_RUN_TAG | (run - 1)));
        run = 0;
    }
        ...
        if (run > 0) {
            ...
            if (tag == QOI_OP_RGB_TAG) {
                ...
                if (tag_type == QOI_OP_INDEX_TAG) {
                    ...
                    int dg = (tag & 0x3f) - 32;
                    int dr_dg = (static_cast<int>(tag2 >> 4) & 0x0f) - 8;
                    int db_dg = (static_cast<int>(tag2) & 0x0f) - 8;
                    int nr = static_cast<int>(r) + dg + dr_dg;
                    int ng = static_cast<int>(g) + dg;
                    int nb = static_cast<int>(b) + dg + db_dg;
